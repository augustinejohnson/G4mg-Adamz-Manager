...
import { 
  ...
  Download, Filter, ChevronDown, Check, Truck, ShoppingCart, CheckCircle2, XCircle, UploadCloud, DownloadCloud, Camera
} from 'lucide-react';
import { 
  AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, Cell
} from 'recharts';
import { addProduct, updateProduct, deleteProduct, addStockMovement, addWarehouse, deleteWarehouse, addSupplier, deleteSupplier, addPurchaseOrder, updatePurchaseOrder, deletePurchaseOrder, updateEmployee, addBranchOrder, updateBranchOrder, deleteBranchOrder, updateB2BOrder, uploadFile } from './firebase';
...
export default function InventoryModule({ 
  ...
  // Form states
  const initialProductState = { name: '', genericName: '', description: '', category: '', subCategory: '', sku: '', barcode: '', costPrice: '', priceStaff: '', priceDistributor: '', priceWholesale: '', batchNumber: '', expiryDate: '', image: '', stockByWarehouse: {}, minStockLevel: '10', supplierId: '', initialQty: '', initialWarehouseId: '' };
  ...
  const lookupBarcodeOnline = async (barcode) => {
    ...
    try {
      ...
      // Fallback: try UPC item DB via proxy-friendly approach
      return null;
    } catch (err) {
  ...
  const handleInventoryScan = async (decodedText) => {
    setShowInventoryScanner(false);
    
    if (scannerTarget === 'search') {
      const existsLocally = products.find(p => p.barcode === decodedText || p.sku === decodedText);
      if (existsLocally) {
        setSearchQuery(decodedText);
        setActiveTab('products');
      } else {
        // Not found locally. Try online.
        setSearchQuery('');
        setActiveTab('products');
        
        // Open add form with loading state
        setNewProduct({ ...initialProductState, barcode: decodedText });
        setIsAddProductOpen(true);
        
        const found = await lookupBarcodeOnline(decodedText);
        if (found) {
           setNewProduct(prev => ({
             ...prev,
             name: found.name,
           ...
           alert("Product not found online. Please enter details manually.");
        }
      }
    } else if (scannerTarget === 'form') {
      // Fill barcode field
      if (isEditProductOpen) {
        setEditProductData(prev => ({ ...prev, barcode: decodedText }));
      } else {
        setNewProduct(prev => ({ ...prev, barcode: decodedText }));
      }
      // Try online lookup to auto-fill other fields
      const found = await lookupBarcodeOnline(decodedText);
      if (found) {
        if (isEditProductOpen) {
          setEditProductData(prev => ({
            ...
            category: prev.category || found.category,
            image: prev.image || found.image,
            genericName: prev.genericName || found.genericName,
  ...
  const handleImageUpload = async (e) => {
    ...
    setUploadProgress(0);
    try {
      const path = `organizations/${currentTenant}/product-images/${Date.now()}_${file.name}`;
      const downloadURL = await uploadFile(file, path, (pct) => setUploadProgress(pct));
      if (isEditProductOpen) {
        setEditProductData(prev => ({ ...prev, image: downloadURL }));
      } else {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      const targetWarehouseId = initialWarehouseId || (warehouses.length > 0 ? warehouses[0].id : 'global');
      if (Number(initialQty) > 0) {
        productData.stockByWarehouse = { [targetWarehouseId]: Number(initialQty) };
      }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex gap-1.5 bg-slate-100 p-1 rounded-2xl w-full overflow-x-auto no-scrollbar mb-3">
        {[
          ...
          ...(isPremium ? [{ id: 'movements', icon: ArrowRightLeft, label: 'Moves' }] : []),
          ...(isPremium ? [{ id: 'purchasing', icon: ShoppingCart, label: 'Purchase' }] : []),
          ...(isPremium ? [{ id: 'b2b_orders', icon: ShoppingCart, label: 'B2B' }] : []),
          ...(isPremium ? [{ id: 'warehouses', icon: Building, label: 'Warehouse' }] : []),
          ...(isPremium ? [{ id: 'branch_orders', icon: Truck, label: 'Requisition' }] : []),
          ...(isPremium ? [{ id: 'suppliers', icon: Truck, label: 'Suppliers' }] : [])
        ].map(tab => (
          ...
          <div className="space-y-6 animate-in fade-in">
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                <input type="text" placeholder="Search products, SKU, barcode..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-white border border-slate-100 rounded-2xl outline-none focus:border-recloud-500 shadow-sm text-sm" />
              </div>
              <button onClick={() => { setScannerTarget('search'); setShowInventoryScanner(true); }} className="bg-slate-800 hover:bg-slate-700 text-white px-4 py-3 rounded-2xl font-bold flex items-center gap-2 transition-colors shadow-sm whitespace-nowrap text-sm">
            ...
            <form onSubmit={isEditProductOpen ? handleEditProduct : handleAddProduct} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input type="text" value={isEditProductOpen ? editProductData.barcode : newProduct.barcode} onChange={e => isEditProductOpen ? setEditProductData({...editProductData, barcode: e.target.value}) : setNewProduct({...newProduct, barcode: e.target.value})} className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-recloud-500" />
                    <button type="button" onClick={() => { setScannerTarget('form'); setShowInventoryScanner(true); }} className="bg-slate-800 hover:bg-slate-700 text-white px-3 py-2 rounded-xl transition-colors flex items-center" title="Scan barcode">
                      <Camera className="w-4 h-4" />
                    </button>
                  </div>
                  {isLookingUpBarcode && <p className="text-xs text-blue-500 mt-1 animate-pulse font-semibold">🔍 Looking up product info online...</p>}
                </div>
                <div>
              ...
              <div>
                ...
                <div className="flex gap-2 items-center">
                  <input type="url" placeholder="Paste URL or upload below" value={isEditProductOpen ? editProductData.image : newProduct.image} onChange={e => isEditProductOpen ? setEditProductData({...editProductData, image: e.target.value}) : setNewProduct({...newProduct, image: e.target.value})} className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-recloud-500" />
                  <label className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2.5 rounded-xl cursor-pointer transition-colors flex items-center gap-1 text-xs font-bold whitespace-nowrap">
                    <UploadCloud className="w-4 h-4" /> {isUploadingImage ? `${uploadProgress}%` : 'Upload'}
                    <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" disabled={isUploadingImage} />
                  </label>
                </div>
                {(isEditProductOpen ? editProductData.image : newProduct.image) && (
                  <div className="mt-2 flex items-center gap-2">
                    <img src={isEditProductOpen ? editProductData.image : newProduct.image} alt="Preview" className="w-12 h-12 rounded-lg object-cover border border-slate-200" onError={(e) => e.target.style.display='none'} />
                    <span className="text-xs text-emerald-600 font-semibold">✓ Image set</span>
                  </div>
                )}
                {isUploadingImage && (
                  <div className="mt-2 w-full bg-slate-100 rounded-full h-2"><div className="bg-emerald-500 h-2 rounded-full transition-all" style={{width: `${uploadProgress}%`}}></div></div>
                )}
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-4 border-b border-slate-100">
              <h3 className="text-lg font-black text-slate-800">{scannerTarget === 'search' ? 'Scan to Search' : 'Scan Barcode'}</h3>
              <button onClick={() => setShowInventoryScanner(false)} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-4">
              <ScannerComponent onScanSuccess={(text) => handleInventoryScan(text)} onScanError={(err) => console.log(err)} />
            </div>
          </div>
        </div>
      )}
