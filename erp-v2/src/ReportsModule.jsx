...
import { Filter, DollarSign, TrendingUp, Users, ShoppingBag, Calendar, Download, Activity, FileText } from 'lucide-react';
import { jsPDF } from 'jspdf';
import autoTable from 'jspdf-autotable';

export default function ReportsModule({ sales, b2bOrders, customers, products, employees, invoices, ledger, expenses, currentIndustry }) {
  ...
  const filteredExpenses = useMemo(() => {
    return (expenses || []).filter(exp => {
      const expDate = new Date(exp.date || exp.createdAt?.seconds * 1000 || Date.now());
      if (dateRange.start && expDate < new Date(dateRange.start)) return false;
      if (dateRange.end) {
        const endDay = new Date(dateRange.end);
        endDay.setHours(23, 59, 59, 999);
        if (expDate > endDay) return false;
      }
      return true;
    });
  }, [expenses, dateRange]);
  ...
  const handleExportCsv = () => {
    ...
    const rows = filteredOrders.map(order => [
      ...
      `"${order.customer}"`,
      `"${order.staff}"`,
      `"${(order.items || []).map(i => `${i.qty || i.quantity || 1}x ${i.name || i.description || ''}`).join(', ')}"`,
      order.totalAmount
    ]);
  ...
  const handleExportPdf = () => {
    ...
    doc.text(`Total Revenue: NGN ${totalRevenue.toLocaleString()}`, 14, 36);
    doc.text(`Total Records: ${filteredOrders.length}`, 14, 42);

    const tableColumn = ["Invoice #", "Date", "Customer", "Channel", "Items", "Amount"];
    const tableRows = [];
  ...
  const monthlyData = useMemo(() => {
    ...
    filteredExpenses.forEach(exp => {
      const monthYear = new Date(exp.date || exp.createdAt?.seconds * 1000 || Date.now()).toLocaleString('default', { month: 'short', year: 'numeric' });
      if (!monthsMap[monthYear]) {
        monthsMap[monthYear] = { name: monthYear, posSales: 0, b2bSales: 0, invoiced: 0, expenses: 0, total: 0 };
      }
      monthsMap[monthYear].expenses += Number(exp.amount) || 0;
    });
    ...
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden">
      ...
      <div className="bg-white border-b border-slate-200 px-4 md:px-8 py-4 md:py-6 shrink-0 flex flex-col md:flex-row justify-between items-start md:items-center z-10 shadow-sm relative gap-3">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <button onClick={handleExportPdf} className="bg-white border-2 border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 font-bold py-2 px-4 rounded-xl shadow-sm transition-all flex items-center gap-2 text-xs md:text-sm flex-1 md:flex-none justify-center">
            <FileText className="w-4 h-4" /> Export PDF
          </button>
          <button onClick={handleExportCsv} className="bg-recloud-600 hover:bg-recloud-700 text-white font-bold py-2 px-4 rounded-xl shadow-sm transition-all flex items-center gap-2 text-xs md:text-sm flex-1 md:flex-none justify-center">
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-2 gap-3 md:gap-6 mb-4 md:mb-8">
           <div className="bg-white p-3 md:p-6 rounded-xl md:rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3 md:gap-4 overflow-hidden">
            <div className="w-9 h-9 md:w-12 md:h-12 rounded-full bg-red-100 flex items-center justify-center shrink-0">
              <FileText className="w-6 h-6 text-red-600" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[10px] md:text-sm font-bold text-slate-500 uppercase tracking-wider truncate" title="Total Expenses">Total Expenses</p>
              <p className="text-lg md:text-2xl font-black text-slate-800 truncate" title={`₦${totalExpenses.toLocaleString()}`}>₦{totalExpenses.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
            </div>
          </div>
          
          <div className="bg-white p-3 md:p-6 rounded-xl md:rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3 md:gap-4 overflow-hidden">
            <div className={`w-9 h-9 md:w-12 md:h-12 rounded-full flex items-center justify-center shrink-0 ${netProfit >= 0 ? 'bg-emerald-100' : 'bg-red-100'}`}>
              <Activity className={`w-6 h-6 ${netProfit >= 0 ? 'text-emerald-600' : 'text-red-600'}`} />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[10px] md:text-sm font-bold text-slate-500 uppercase tracking-wider truncate" title="Net Profit">Net Profit</p>
              <p className={`text-lg md:text-2xl font-black truncate ${netProfit >= 0 ? 'text-emerald-600' : 'text-red-600'}`} title={`₦${netProfit.toLocaleString()}`}>₦{netProfit.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
            </div>
          </div>
        </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  {currentIndustry === 'law_firm' ? (
                    <Bar dataKey="invoiced" name="Invoiced Revenue" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                  ) : (
                    <>
                      <Bar dataKey="posSales" name="POS Retail Sales" stackId="a" fill="#3b82f6" radius={[0, 0, 0, 0]} />
