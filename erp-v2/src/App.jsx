...
, History, Database, Webhook, ShieldCheck, Landmark, Lock, Crown } from 'lucide-react';
...
const SubscriptionModule = React.lazy(() => import('./SubscriptionModule'));
const SuperAdminModule = React.lazy(() => import('./SuperAdminModule'));
...
const FEATURES_DATA = [
  {
    ...
    description: 'Track opposing parties, counsels, court dates, and statutes of limitations with automated deadline indicators.',
    tools: [
      { name: 'Opposing Party Tracker', desc: 'Maintain detailed records of opposing counsels, parties, and their contact information across all active cases.' },
      { name: 'Statute of Limitations Calculator', desc: 'Automatically calculate and alert you of impending deadlines based on jurisdiction rules.' },
      { name: 'Court Date Scheduler', desc: 'Sync court appearances, depositions, and mediation dates directly to your calendar.' },
      { name: 'Document Assembly', desc: 'Generate standard legal forms, motions, and pleadings automatically populated with case data.' }
    ]
  },
  {
    id: 'time-billing',
    ...
    description: 'Log billable hours by task and rate. Automatically generate professional invoices for unbilled time with one click.',
    tools: [
      { name: 'One-Click Timers', desc: 'Start and stop multiple concurrent timers across different client matters.' },
      { name: 'LEDES Billing Export', desc: 'Export invoices in LEDES format for institutional clients.' },
      { name: 'Batch Invoicing', desc: 'Generate hundreds of invoices at the end of the month with a single click.' },
      { name: 'Expense Tracking', desc: 'Capture soft and hard costs (printing, filing fees, postage) directly to client matters.' }
    ]
  },
  {
    id: 'trust-accounting',
    ...
    description: 'Manage client retainers compliantly. Track trust liabilities separately and transfer earned fees to operations securely.',
    tools: [
      { name: 'Three-Way Reconciliation', desc: 'Ensure compliance with state bar rules by reconciling bank, book, and client ledger balances.' },
      { name: 'Evergreen Retainer Alerts', desc: 'Automatically notify clients when their trust balance falls below a predefined minimum.' },
      { name: 'Trust-to-Operating Transfers', desc: 'Securely move earned fees from trust to operating accounts with full audit trails.' },
      { name: 'Client Ledger Reports', desc: 'Generate detailed, client-facing reports of all trust account activity.' }
    ]
  },
  {
    id: 'conflict-checker',
    icon: 'ShieldCheck',
    color: 'amber',
    title: 'Global Conflict Checker',
    description: 'Maintain ethical compliance by deep-searching across all clients, cases, and opposing parties before taking a matter.',
    tools: [
      { name: 'Phonetic Search Algorithm', desc: 'Find potential conflicts even if names are misspelled or use common variations.' },
      { name: 'Corporate Family Tree Search', desc: 'Identify conflicts across parent companies, subsidiaries, and affiliates.' },
      { name: 'Conflict Resolution Workflows', desc: 'Document the review process and store signed conflict waivers directly in the system.' },
      { name: 'Cross-Database Scanning', desc: 'Search across active matters, archived cases, and rejected prospects simultaneously.' }
    ]
  },
  {
    id: 'retail-erp',
    icon: 'Users',
    color: 'recloud',
    title: 'Retail & General ERP',
    description: 'Not a law firm? Manage HR, Shifts, Payroll, B2B Inventory, and general Accounting in one unified workspace.',
    tools: [
      { name: 'Multi-Warehouse Inventory', desc: 'Track stock levels across multiple locations with low-stock alerts.' },
      { name: 'Point of Sale (POS)', desc: 'Process transactions, apply discounts, and manage split payments.' },
      { name: 'B2B Wholesale Portal', desc: 'Allow corporate clients to log in, view custom pricing, and place bulk orders.' },
      { name: 'Shift Scheduling', desc: 'Create and distribute employee schedules, track attendance, and process payroll.' }
    ]
  },
  {
    id: 'security',
    ...
    description: 'Tenant-isolated architecture with granular role-based access control and firestore security rules to protect your data.',
    tools: [
      { name: 'Role-Based Access Control', desc: 'Create custom roles with specific permissions for different staff members.' },
      { name: 'Audit Logging', desc: 'Track every action taken in the system: who did what and when.' },
      { name: 'Tenant Isolation', desc: 'Your data is completely segregated from other organizations on the platform.' },
      { name: 'Multi-Factor Authentication', desc: 'Require a second form of verification for all administrative accounts.' }
    ]
  }
];
  ...
  render() {
    if (this.state.hasError) {
      if (this.state.error?.message?.includes('Failed to fetch dynamically imported module')) {
        ...
        return <div className="p-4 text-center text-slate-500 flex flex-col items-center justify-center h-full gap-4">
          <div className="w-8 h-8 border-4 border-recloud-600 border-t-transparent rounded-full animate-spin"></div>
          <p>Updating application to the latest version...</p>
        </div>;
      }
      return (
        <div className="p-4 bg-red-50 text-red-500 rounded-xl m-4">
          ...
          <button onClick={() => window.location.reload()} className="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Reload Page</button>
        </div>
      );
    }
...
export default function App() {
  ...
  const [regStep, setRegStep] = useState('form'); // 'form' or 'verification'
  ...
  const [settingsForm, setSettingsForm] = useState({ companyName: '', logoUrl: '', address: '', phone: '', notificationEmail: '', bankName: '', accountName: '', accountNumber: '', paystackLiveKey: '', industry: 'retail' });
  ...
  useEffect(() => {
    ...
    const unsubs = [
      subscribeToCollection(currentTenant, 'employees', setEmployees),
      subscribeToCollection(currentTenant, 'shifts', setShifts),
      subscribeToCollection(currentTenant, 'products', setProducts),
      subscribeToCollection(currentTenant, 'sales', setSales),
      subscribeToCollection(currentTenant, 'ledger', setLedger),
      subscribeToCollection(currentTenant, 'invoices', setInvoices),
      subscribeToCollection(currentTenant, 'customers', setCustomers),
      subscribeToCollection(currentTenant, 'deals', setDeals),
      subscribeToCollection(currentTenant, 'expenses', setExpenses),
      subscribeToCollection(currentTenant, 'stockMovements', setStockMovements),
      subscribeToCollection(currentTenant, 'warehouses', setWarehouses),
      subscribeToCollection(currentTenant, 'purchaseOrders', setPurchaseOrders)
    ];
  ...
  const fetchData = async (tenant = currentTenant) => {
    ...
    try {
      ...
      if (error.message && (error.message.includes('offline') || error.message.includes('Failed to get document'))) {
        setDataFetchError('Network connection blocked. Please disable any adblockers (like Brave Shields), check your internet connection, and refresh the page.');
      } else {
        setDataFetchError('Failed to load application data. Please check your connection and try again.');
      }
    } finally {
      setIsLoading(false);
    }
      ...
      getTenantConfig(currentTenant).then(configData => {
        if (configData) {
          ...
          setSettingsForm({
            ...
            paystackLiveKey: configData.paystackLiveKey || '',
  ...
  const getNotifications = () => {
    ...
    if (currentUser?.role === 'admin') {
      ...
      // Add notifications for sales made today
      const todayString = new Date().toISOString().slice(0, 10);
      sales.forEach(sale => {
        const saleDateObj = sale.createdAt?.toDate ? sale.createdAt.toDate() : (sale.createdAt ? new Date(sale.createdAt) : new Date(sale.date));
        if (saleDateObj && saleDateObj.toISOString().slice(0, 10) === todayString) {
          notifs.push({
            id: `sale_${sale.id}`,
            type: 'sale',
            text: `New sale of ₦${Number(sale.totalAmount || 0).toLocaleString()} by ${sale.createdBy || 'Cashier'}`,
  ...
  const processUserWorkspaces = async (user) => {
    ...
    if (verifiedWorkspaces.length === 0) {
      if (user.email === 'ronimationstudios@gmail.com') {
        try {
          await registerTenant('Ronimation Studios', 'ronimationstudios', user.email, 'ignored', '', '', '', 'retail');
          await loadWorkspace('ronimationstudios', user.uid);
          return;
        } catch (e) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // If the user isn't already authenticated with this email, create them
      ...
      setAuthMessage({ type: 'success', text: `A verification link has been sent to ${regEmail}. Please check your inbox and spam folder.` });
      
    } catch (err) {
      setAuthMessage({ type: 'error', text: err.message || 'Error sending verification email.' });
      console.error("Verification Setup Error: ", err);
    }
    setIsLoading(false);
  };

  const handleVerifyEmailAndFinish = async () => {
    setIsLoading(true);
    setAuthMessage({ type: '', text: '' });
    try {
      const user = auth.currentUser;
      if (!user) {
         throw new Error("User session lost. Please reload the page.");
      }
      ...
      if (!user.emailVerified) {
         throw new Error("Email is not verified yet. Please click the link in the email we sent you, then try again.");
      }
      
      const slug = regWorkspace.toLowerCase().replace(/[^a-z0-9]/g, '');
      await registerTenant(regCompany, slug, regEmail, 'ignored', '', '', '', regIndustry);
      ...
      setAuthMessage({ type: 'error', text: err.message || 'Error finishing registration.' });
    }
    setIsLoading(false);
  };
  ...
  if (!currentUser) {
    if (publicView === 'landing') {
      return (
        <div className="min-h-screen bg-slate-50 font-sans flex flex-col overflow-y-auto">
          ...
          <div className="bg-white border-b border-slate-200 sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
              ...
              <div className="flex gap-6 items-center">
                <button onClick={() => setPublicView('about')} className="text-sm font-bold text-slate-600 hover:text-recloud-600 transition-colors">About Platform</button>
          ...
          {/* Centered Hero */}
          <div className="flex-1 flex flex-col items-center justify-center text-center max-w-7xl mx-auto px-6 py-12 min-h-[60vh]">
            <h1 className="text-5xl lg:text-7xl font-bold text-slate-800 tracking-tight leading-tight mb-6">
              The Operating System for <span className="text-transparent bg-clip-text bg-gradient-to-r from-recloud-600 to-blue-500 whitespace-nowrap">Modern Teams.</span>
            </h1>
            <p className="text-base md:text-lg font-medium text-slate-600 leading-relaxed mb-8 max-w-2xl">
              Manage HR, shifts, payroll, and recruitment in one unified workspace. Start your free organization today.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button onClick={() => setPublicView('register')} className="bg-recloud-600 hover:bg-recloud-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg shadow-recloud-500/30 transition-all transform hover:scale-105 text-base">
                Register Company & Start 7-Day Trial
              </button>
              <button onClick={() => setPublicView('careers')} className="bg-white hover:bg-slate-50 border-2 border-slate-200 text-slate-700 font-bold py-3 px-6 rounded-xl shadow-sm transition-all text-base hover:border-slate-300">
                View Demo Access
              </button>
            </div>
          </div>
          ...
          {/* Footer */}
          <div className="py-6 text-center text-base font-medium text-slate-500 mt-auto border-t border-slate-200">
            <p className="flex flex-col sm:flex-row items-center justify-center gap-1">
              Questions or Support? WhatsApp us at 
              <a href="https://wa.me/2348124580183" target="_blank" rel="noopener noreferrer" className="text-emerald-600 font-bold hover:underline flex items-center gap-1 text-lg">
                +2348124580183
              </a>
            </p>
            <p className="mt-2 text-sm text-slate-400">Developed by <span className="font-bold text-slate-600 text-lg">Ronimation Studios</span></p>
          </div>
        </div>
      );
    }
    ...
    if (publicView === 'register') {
      const getIconComponent = (iconName, className) => {
        const IconComponent = { Briefcase, Clock, Landmark, ShieldCheck, Users, Lock }[iconName] || Briefcase;
      ...
      const getBgColor = (color) => {
        const map = {
          ...
          recloud: 'bg-recloud-100 text-recloud-600',
      ...
      return (
        <div className="min-h-screen bg-slate-50 font-sans flex flex-col h-screen overflow-y-auto">
          ...
          <div className="bg-white border-b border-slate-200 sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
              <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setPublicView('landing')} title="Back to Landing Page">
                <div className="bg-gradient-to-tr from-recloud-600 to-recloud-400 p-2 rounded-xl shadow-lg shadow-recloud-500/30 group-hover:opacity-90 transition-opacity">
                  <Building2 className="text-white w-5 h-5"/>
                </div>
                <h1 className="text-xl font-bold text-slate-800 tracking-tight group-hover:text-recloud-600 transition-colors">Recloud ERP</h1>
              </div>
          ...
          <div className="flex-1 flex flex-col lg:flex-row items-center justify-center max-w-6xl mx-auto px-6 py-12 gap-12 w-full">
            <div className="flex-1 space-y-6 max-w-md hidden lg:block">
              <h1 className="text-5xl font-bold text-slate-800 tracking-tight leading-tight">
                Start your <span className="text-transparent bg-clip-text bg-gradient-to-r from-recloud-600 to-blue-500">Free Workspace</span>
              </h1>
              <p className="text-xl text-slate-500 leading-relaxed">
                Join thousands of modern teams managing their operations in one unified ecosystem.
              </p>
            </div>
            ...
            <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl text-center">
              <Mail className="w-16 h-16 text-recloud-500 mx-auto mb-6" />
              <h2 className="text-2xl font-black text-slate-800 mb-4">Verify your email</h2>
              <p className="text-slate-600 mb-4">
                We've sent a verification link to <strong>{regEmail}</strong>. 
                Please check your inbox (and spam folder), click the link, and then return here to finish setting up your workspace.
              </p>
              <div className="bg-amber-50 text-amber-700 text-sm font-medium p-3 rounded-lg mb-8 border border-amber-200">
                ⚠️ Do not close this window! Return here after clicking the link.
              </div>
              
              {authMessage.text && (
                <div className={`p-4 rounded-xl mb-6 text-sm font-medium ${authMessage.type === 'error' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                  {authMessage.text}
                </div>
              )}
              ...
              <button 
                ...
                disabled={isLoading}
                className="w-full py-3.5 px-6 rounded-xl font-bold bg-recloud-600 hover:bg-recloud-700 text-white transition-all shadow-md hover:shadow-lg disabled:opacity-70 flex items-center justify-center gap-2 mb-4"
              >
                {isLoading ? <span className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></span> : <CheckCircle2 className="w-5 h-5" />}
            ...
            <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl">
              <div className="text-center mb-8">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-recloud-50 text-recloud-600 mb-6">
                  <Building2 className="w-8 h-8" />
                </div>
                <h2 className="text-2xl font-black text-slate-800 tracking-tight">Start your free organization</h2>
                <p className="text-slate-500 mt-2">No credit card required. Setup in minutes.</p>
              </div>
              ...
              <form onSubmit={handleRegister} className="space-y-5">
                ...
                <button type="submit" disabled={isLoading} className="w-full bg-slate-800 hover:bg-slate-900 disabled:bg-slate-400 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all mt-2 flex justify-center items-center gap-2">
                  {isLoading ? 'Creating Workspace...' : 'Register Company & Start 7-Day Premium Trial'}
                </button>
              </form>
            </div>
            )}
        ...
        <div className="min-h-screen bg-slate-50 font-sans flex flex-col overflow-y-auto">
          ...
          <div className="bg-white border-b border-slate-200 sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
              ...
              <div className="flex gap-4 items-center">
                <button onClick={() => setPublicView('register')} className="text-sm font-bold text-slate-600 hover:text-recloud-600 transition-colors">Register Company</button>
                <button onClick={() => setPublicView('login')} className="text-sm font-bold text-slate-600 hover:text-recloud-600 transition-colors">Sign In</button>
              </div>
            </div>
          </div>
          ...
          {/* Features Section */}
          <div className="bg-white border-y border-slate-200 py-16 mt-8">
            <div className="max-w-5xl mx-auto px-6">
              <div className="text-center mb-12">
                <h2 className="text-3xl font-bold text-slate-800 tracking-tight">Everything you need to run your business</h2>
                <p className="text-slate-500 mt-3 text-base">Click on any module below to explore its tools in detail.</p>
              </div>
                ...
                {FEATURES_DATA.map(feature => (
                  <div 
                    ...
                    onClick={() => {
                      ...
                      setPublicView('feature_detail');
                    }}
                    className="p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:shadow-xl hover:shadow-slate-200/50 transition-all group cursor-pointer transform hover:-translate-y-1"
                  >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform ${getBgColor(feature.color)}`}>
                      {getIconComponent(feature.icon, "w-6 h-6")}
                    </div>
                    <h3 className="text-lg font-bold text-slate-800 mb-2 group-hover:text-recloud-600 transition-colors">{feature.title}</h3>
                    <p className="text-sm text-slate-600 leading-relaxed font-medium">{feature.description}</p>
                    <div className="mt-3 text-recloud-600 font-bold text-xs flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      View tools <ChevronDown className="w-4 h-4 -rotate-90"/>
                    </div>
                  </div>
                ))}
              </div>
    ...
    if (publicView === 'feature_detail' && selectedFeature) {
      const getIconComponent = (iconName, className) => {
        const IconComponent = { Briefcase, Clock, Landmark, ShieldCheck, Users, Lock }[iconName] || Briefcase;
        return <IconComponent className={className} />;
      };
      ...
      return (
        <div className="min-h-screen h-screen flex flex-col bg-slate-50 font-sans overflow-y-auto pb-12">
          {/* Header */}
          <div className="bg-white border-b border-slate-200 sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
              <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setPublicView('landing')} title="Back to Landing Page">
          ...
          <div className="max-w-4xl mx-auto px-6 py-12">
            <button 
              ...
              className="flex items-center gap-2 text-slate-500 hover:text-slate-800 transition-colors font-bold mb-8"
            >
              <ChevronDown className="w-5 h-5 rotate-90" /> Back to About Platform
            </button>

            <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="p-10 border-b border-slate-100 bg-slate-50/50">
                <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 shadow-sm ${getBgColor(selectedFeature.color)}`}>
                  {getIconComponent(selectedFeature.icon, "w-10 h-10")}
                </div>
                <h1 className="text-4xl font-bold text-slate-800 mb-4">{selectedFeature.title}</h1>
                <p className="text-xl text-slate-600 leading-relaxed max-w-2xl">{selectedFeature.description}</p>
              </div>
              
              <div className="p-10">
                <h2 className="text-2xl font-bold text-slate-800 mb-8 flex items-center gap-2">
                  <Sparkles className="w-6 h-6 text-recloud-500" /> Included Tools
                </h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {selectedFeature.tools.map((tool, idx) => (
                    <div key={idx} className="p-6 rounded-2xl bg-slate-50 border border-slate-100 hover:border-slate-300 transition-colors shadow-sm">
              ...
              <div className="p-10 bg-slate-800 text-white text-center">
                <h3 className="text-2xl font-bold mb-4">Ready to get started?</h3>
                <p className="text-slate-400 mb-6">Launch your workspace in 30 seconds and try all features for free.</p>
                <button onClick={() => setPublicView('register')} className="bg-recloud-500 hover:bg-recloud-600 text-white font-bold py-3 px-8 rounded-xl shadow-lg transition-all">
                  Create Workspace
                </button>
              </div>
            </div>
    ...
    return (
      <div className="flex min-h-[100dvh] w-full bg-slate-50 items-center justify-center font-sans relative overflow-y-auto py-12">
        ...
        {/* Footer */}
        <div className="absolute bottom-0 w-full py-6 text-center text-base font-medium text-slate-500 border-t border-slate-200 bg-slate-50">
          <p className="flex flex-col sm:flex-row items-center justify-center gap-1">
            Questions or Support? WhatsApp us at 
            <a href="https://wa.me/2348124580183" target="_blank" rel="noopener noreferrer" className="text-emerald-600 font-bold hover:underline flex items-center gap-1 text-lg">
              +2348124580183
  ...
  const isSuperAdmin = currentUser?.email === 'ronimationstudios@gmail.com';
  const isSubscriptionActive = () => {
    if (!tenantConfig) return true; // Let it load
    const status = tenantConfig.subscriptionStatus;
    
    if (status === 'active') {
      if (!tenantConfig.subscriptionEndsAt) return true; // Lifetime or old account
      const endsAt = new Date(tenantConfig.subscriptionEndsAt);
      if (endsAt > new Date()) return true;
      return false; // Subscription expired!
    }
    
    if (status === 'trial') {
      if (!tenantConfig.trialEndsAt) return true;
      const endsAt = new Date(tenantConfig.trialEndsAt);
      if (endsAt > new Date()) return true;
    }
    
    return false; // inactive or expired trial
  };

  if (tenantConfig && !isSubscriptionActive()) {
    return (
      <React.Suspense fallback={<div className="flex h-screen items-center justify-center bg-slate-50"><div className="w-8 h-8 border-4 border-recloud-500 border-t-transparent rounded-full animate-spin"></div></div>}>
        <SubscriptionModule currentTenant={currentTenant} tenantConfig={tenantConfig} currentUser={currentUser} refreshData={() => fetchData(currentTenant)} />
      </React.Suspense>
    );
  }
  ...
  const isPremium = (() => {
    if (!tenantConfig) return true; // Let it load, don't restrict
    const plan = tenantConfig.subscriptionPlan;
    const status = tenantConfig.subscriptionStatus;
    // Trial gets full premium access, lifetime gets premium, explicit premium gets premium
    if (status === 'trial') return true;
    if (plan === 'premium' || plan === 'lifetime') return true;
    return false; // 'basic' or undefined
  })();
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-gradient-to-br from-indigo-50 via-white to-purple-50 overflow-hidden font-sans relative">
      ...
      <>
        ...
        <aside className={`fixed md:relative top-0 left-0 h-full w-64 md:w-56 bg-slate-800 transform text-slate-300 flex flex-col justify-between shadow-2xl z-[60] md:z-10 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
        ...
        <div className="flex-1 overflow-y-auto pb-4 custom-scrollbar">
          ...
          <nav className="px-3 space-y-1">
            <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('launcher'); }} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-bold text-white bg-white/10 hover:bg-white/20 transition-all duration-200 mb-2 border border-white/5">
            ...
            {['admin', 'hr_manager', 'sales_manager', 'inventory_manager', 'accountant'].includes(currentUser.role) && (
              <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('analytics'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'analytics' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                <LayoutDashboard className="w-5 h-5" /> Executive Dashboard
              </button>
            )}
            
            {['admin', 'hr_manager'].includes(currentUser.role) && (
              <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('hr'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'hr' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner border border-recloud-500/20' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                <Users className="w-5 h-5" /> Human Resources
              </button>
            )}

            {isPremium && ['admin', 'sales_manager'].includes(currentUser.role) && isModuleEnabled('crm') && (
              <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('crm'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'crm' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
            ...
            {['admin', 'inventory_manager'].includes(currentUser.role) && isModuleEnabled('inventory') && (
              <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('inventory'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'inventory' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                <Package className="w-5 h-5" /> Inventory
              </button>
            )}

            {['admin', 'sales_manager', 'cashier', 'sales_rep'].includes(currentUser.role) && isModuleEnabled('pos') && (
              <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('pos'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'pos' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                <Banknote className="w-5 h-5" /> Point of Sale
              </button>
            )}

            {isPremium && ['admin', 'accountant'].includes(currentUser.role) && (
              <>
                <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('accounting'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'accounting' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
            ...
            {['admin', 'hr_manager', 'sales_manager', 'inventory_manager', 'accountant', 'staff'].includes(currentUser.role) && (
              <>
                {isPremium && (
                  <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('documents'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'documents' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                    <FolderOpen className="w-5 h-5" /> Documents
                  </button>
                )}
                {isPremium && (
                  <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('projects'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'projects' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                    <Kanban className="w-5 h-5" /> {currentIndustry === 'law_firm' ? 'Cases & Matters' : 'Projects'}
                  </button>
                )}
                {isPremium && (
                  <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('discuss'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'discuss' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
            ...
            {['admin', 'super_admin'].includes(currentUser.role) && (
              <>
                {['admin', 'super_admin'].includes(currentUser?.role) && (
                  <>
                    <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('history'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'history' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                      <History className={`w-5 h-5 ${activeTab === 'history' ? 'text-recloud-400' : 'text-slate-400'}`} /> Recycle Bin
                    </button>
                    <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('api'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'api' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                      <Settings className={`w-5 h-5 ${activeTab === 'api' ? 'text-recloud-400' : 'text-slate-400'}`} /> API Settings
                    </button>
                    <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('backend'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'backend' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                      <Database className={`w-5 h-5 ${activeTab === 'backend' ? 'text-recloud-400' : 'text-slate-400'}`} /> Backend Manager
                    </button>
                    {currentUser?.email === 'ronimationstudios@gmail.com' && (
                      <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('superadmin'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'superadmin' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                        <ShieldCheck className={`w-5 h-5 ${activeTab === 'superadmin' ? 'text-recloud-400' : 'text-slate-400'}`} /> Super Admin
                      </button>
                    )}
                    <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('webhooks'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'webhooks' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                      <Webhook className={`w-5 h-5 ${activeTab === 'webhooks' ? 'text-recloud-400' : 'text-slate-400'}`} /> Webhooks
                    </button>
                    <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('subscription'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'subscription' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
                      <Landmark className={`w-5 h-5 ${activeTab === 'subscription' ? 'text-recloud-400' : 'text-slate-400'}`} /> Subscription
                    </button>
                  </>
                )}
                {isPremium && (
                <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('ai'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'ai' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner' : 'hover:bg-slate-800/50 hover:text-white'}`}>
            ...
            {['distributor', 'sales_rep', 'b2b_customer'].includes(currentUser.role) && isModuleEnabled('b2b') && (
              <button onClick={() => { setIsMobileMenuOpen(false); setActiveTab('b2b'); }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-200 ${activeTab === 'b2b' ? 'bg-recloud-500/20 text-recloud-300 shadow-inner border border-recloud-500/20' : 'hover:bg-slate-800/50 hover:text-white'}`}>
        ...
        <div className="p-4 border-t border-slate-700/50">
          {['admin', 'super_admin'].includes(currentUser.role) && (
            <button onClick={() => { setActiveTab('hr'); setHrTab('settings'); }} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs md:text-sm font-medium hover:bg-slate-800/50 hover:text-white transition-all">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-transparent relative">
        ...
        {dataFetchError && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/90 backdrop-blur-sm p-4">
            <div className="bg-red-50 border border-red-200 text-red-700 p-8 rounded-2xl max-w-md w-full text-center shadow-2xl">
              <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <XCircle className="w-8 h-8 text-red-500" />
              </div>
              <h2 className="text-xl font-black mb-2">Connection Blocked</h2>
              <p className="text-sm text-red-600 mb-6">{dataFetchError}</p>
              <button onClick={() => window.location.reload()} className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-xl w-full transition-all">
                Reload Page
              </button>
            </div>
          </div>
        ...
        {activeTab !== 'launcher' && (
        <header className="h-16 shrink-0 glass flex items-center justify-between px-4 md:px-8 z-[100] gap-2 md:gap-4">
          <div className="flex items-center gap-2 md:gap-4 flex-1 md:flex-none md:w-[500px] relative">
            ...
            {tenantConfig?.subscriptionStatus === 'trial' && (
              <div 
                onClick={() => setActiveTab('subscription')}
                className="hidden md:flex items-center gap-1.5 bg-gradient-to-r from-orange-500 to-amber-500 text-white px-2.5 py-1 rounded-full shadow-sm cursor-pointer hover:shadow-md transition-all group border border-orange-400/50"
                title="Premium Trial Active"
              >
                <Crown className="w-3 h-3" />
                <span className="text-[9px] font-bold uppercase tracking-wider whitespace-nowrap">
                  {Math.max(0, Math.ceil((new Date(tenantConfig.trialEndsAt) - new Date()) / (1000 * 60 * 60 * 24)))} Days Left
                </span>
              </div>
            )}
          </div>
          <div className="flex items-center gap-2 md:gap-4 relative">
            {isOffline && (
              <div className="flex items-center gap-1.5 bg-slate-800 text-white px-2.5 py-1 rounded-full shadow-sm">
                <span className="w-2 h-2 bg-slate-400 rounded-full animate-pulse"></span>
                <span className="text-[10px] font-bold uppercase tracking-wider whitespace-nowrap">Offline Mode</span>
              </div>
            )}
            ...
            {showNotifications && (
              <div className="absolute top-12 right-0 w-80 bg-white rounded-xl shadow-2xl border border-slate-100 overflow-hidden z-[9999] animate-in fade-in slide-in-from-top-4">
        ...
        <div className="flex-1 flex flex-col overflow-y-auto p-3 pb-24 md:p-8 md:pb-8 relative">
          ...
          {activeTab === 'launcher' && (
            <div className="w-full h-full min-h-[80vh] flex flex-col items-center justify-center animate-in fade-in zoom-in duration-500">
              ...
              <div className="text-center mb-6 md:mb-8 z-10 px-4">
                <h1 className="text-xl md:text-3xl font-black text-slate-800 tracking-tight mb-1 md:mb-2 drop-shadow-sm">
                ...
                <p className="text-[10px] md:text-sm text-slate-500 font-medium">What would you like to focus on today?</p>
              </div>

              <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 md:gap-5 max-w-5xl w-full z-10 px-2 md:px-8">
                
                {isPremium && ['admin', 'hr_manager', 'sales_manager', 'inventory_manager', 'accountant'].includes(currentUser.role) && (
                  <div onClick={() => setActiveTab('analytics')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/70 backdrop-blur-xl shadow-lg shadow-indigo-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1 group-hover:shadow-xl transition-all duration-300 group-hover:scale-105">
                ...
                {['admin', 'sales_manager'].includes(currentUser.role) && isModuleEnabled('crm') && (
                  <div onClick={() => setActiveTab('crm')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/70 backdrop-blur-xl shadow-lg shadow-pink-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-pink-900/10 transition-all duration-300 group-hover:scale-105">
                      <Building2 className="w-5 h-5 md:w-6 md:h-6 text-pink-500" />
                    </div>
                    <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-pink-600 transition-colors text-center leading-tight">
                ...
                {['admin', 'inventory_manager'].includes(currentUser.role) && isModuleEnabled('inventory') && (
                  <div onClick={() => setActiveTab('inventory')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/70 backdrop-blur-xl shadow-lg shadow-orange-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-orange-900/10 transition-all duration-300 group-hover:scale-105">
                      <Package className="w-5 h-5 md:w-6 md:h-6 text-orange-500" />
                    </div>
                    <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-orange-600 transition-colors text-center leading-tight">Inventory</span>
                  </div>
                )}

                {['admin', 'sales_manager', 'cashier', 'sales_rep'].includes(currentUser.role) && isModuleEnabled('pos') && (
                  <div onClick={() => setActiveTab('pos')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/70 backdrop-blur-xl shadow-lg shadow-emerald-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-emerald-900/10 transition-all duration-300 group-hover:scale-105">
                      <Banknote className="w-5 h-5 md:w-6 md:h-6 text-emerald-500" />
                    </div>
                    <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-emerald-600 transition-colors text-center leading-tight">POS</span>
                  </div>
                )}

                {isPremium && ['admin', 'accountant'].includes(currentUser.role) && (
                  <div onClick={() => setActiveTab('accounting')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/70 backdrop-blur-xl shadow-lg shadow-blue-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-blue-900/10 transition-all duration-300 group-hover:scale-105">
                      <Receipt className="w-5 h-5 md:w-6 md:h-6 text-blue-500" />
                    </div>
                    <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-blue-600 transition-colors text-center leading-tight">Accounting</span>
                  </div>
                )}

                {['admin', 'hr_manager'].includes(currentUser.role) && (
                  <div onClick={() => setActiveTab('hr')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/70 backdrop-blur-xl shadow-lg shadow-purple-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-purple-900/10 transition-all duration-300 group-hover:scale-105">
                      <Users className="w-5 h-5 md:w-6 md:h-6 text-purple-500" />
                    </div>
                    <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-purple-600 transition-colors text-center leading-tight">HR</span>
                  </div>
                )}
                ...
                {isPremium && (
                  <>
                    <div onClick={() => setActiveTab('discuss')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/70 backdrop-blur-xl shadow-lg shadow-sky-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-sky-900/10 transition-all duration-300 group-hover:scale-105 relative">
                      ...
                      <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-sky-600 transition-colors text-center leading-tight">Discuss</span>
                    </div>

                    <div onClick={() => setActiveTab('documents')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/70 backdrop-blur-xl shadow-lg shadow-amber-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-amber-900/10 transition-all duration-300 group-hover:scale-105">
                        <FolderOpen className="w-5 h-5 md:w-6 md:h-6 text-amber-500" />
                      </div>
                      <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-amber-600 transition-colors text-center leading-tight">Documents</span>
                    </div>

                    <div onClick={() => setActiveTab('projects')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                      <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-white/70 backdrop-blur-xl shadow-xl shadow-teal-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1.5 group-hover:shadow-2xl group-hover:shadow-teal-900/10 transition-all duration-300 group-hover:scale-105">
                        <Kanban className="w-5 h-5 md:w-6 md:h-6 text-teal-500" />
                      </div>
                      <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-teal-600 transition-colors text-center leading-tight">
                ...
                {['admin', 'super_admin'].includes(currentUser.role) && (
                  <div onClick={() => setActiveTab('ai')} className="group flex flex-col items-center gap-2 md:gap-3 cursor-pointer">
                    <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-white/70 backdrop-blur-xl shadow-xl shadow-indigo-900/5 flex items-center justify-center border border-white/50 group-hover:-translate-y-1.5 group-hover:shadow-2xl group-hover:shadow-indigo-900/10 transition-all duration-300 group-hover:scale-105 relative">
                      <div className="absolute inset-0 bg-gradient-to-tr from-indigo-500/10 to-purple-500/10 rounded-[1.5rem]"></div>
                      <Sparkles className="w-5 h-5 md:w-6 md:h-6 text-indigo-500" />
                    </div>
                    <span className="font-bold text-[10px] md:text-xs text-slate-700 group-hover:text-indigo-600 transition-colors">AI</span>
                  </div>
                )}
          ...
          {activeTab === 'hr' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-4 md:mb-8 relative z-10 gap-3">
                ...
                <button onClick={() => {
                  if (!isPremium && visibleEmployees.length >= 1) {
                    alert("Basic plan is limited to 1 employee. Please upgrade to Premium to add more staff.");
                    return;
                  }
                  setIsAddModalOpen(true);
                }} className="bg-recloud-600 hover:bg-recloud-700 text-white px-4 md:px-5 py-2 md:py-2.5 rounded-xl shadow-lg shadow-recloud-500/30 font-bold text-xs md:text-sm flex items-center gap-2 transition-all transform hover:-translate-y-0.5 w-full md:w-auto justify-center md:justify-start">
              ...
              <div className="flex justify-between items-center mb-4 md:mb-6 border-b border-slate-200/50 relative z-10">
                <div className="flex gap-1 md:gap-4 overflow-x-auto no-scrollbar w-full">
                  ...
                  {isPremium && (currentUser?.role === 'admin' || currentUser?.role === 'super_admin' || currentUser?.role === 'hr_manager') && (
                    <>
                  ...
                  <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm max-w-2xl">
                    <form onSubmit={handleSaveSettings} className="space-y-6">
                      ...
                      <div className="pt-6 mt-6 border-t border-slate-200">
                        <h4 className="text-lg font-bold text-slate-800 mb-2 flex items-center gap-2">📧 Sale Notifications</h4>
                        <p className="text-xs text-slate-500 mb-4">Get an instant email notification every time a sale is completed on the POS.</p>
                        <div>
                          <label className="block text-sm font-bold text-slate-700 mb-2">Notification Email</label>
                          <input type="email" value={settingsForm.notificationEmail} onChange={e => setSettingsForm({...settingsForm, notificationEmail: e.target.value})} className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-recloud-500/20 focus:border-recloud-500 outline-none transition-all" placeholder="admin@yourbusiness.com" />
                          <p className="text-xs text-slate-500 mt-1">Leave blank to disable sale email notifications.</p>
                        </div>
                      </div>
                      <div className="pt-6 mt-6 border-t border-slate-200">
          ...
          <React.Suspense fallback={<div className="h-full flex items-center justify-center bg-slate-50 rounded-2xl border border-slate-200"><div className="w-12 h-12 border-4 border-recloud-600 border-t-transparent rounded-full animate-spin"></div></div>}>
            ...
            {isPremium && activeTab === 'accounting' && (
            ...
            {isPremium && activeTab === 'client_portal' && (
            ...
            {activeTab === 'inventory' && isModuleEnabled('inventory') && (
              <div className="flex flex-col flex-1 w-full bg-slate-50 p-0 overflow-hidden rounded-2xl shadow-sm border border-slate-200">
                <ErrorBoundary><InventoryModule 
                  products={products} stockMovements={stockMovements} warehouses={warehouses} suppliers={suppliers} purchaseOrders={purchaseOrders} branchOrders={branchOrders} employees={employees} currentUser={currentUser} currentTenant={currentTenant} b2bOrders={b2bOrders} isPremium={isPremium} refreshData={() => fetchData(currentTenant)}
                /></ErrorBoundary>
              </div>
            )}
            ...
            {activeTab === 'pos' && isModuleEnabled('pos') && (
              <div className="flex flex-col flex-1 w-full bg-slate-50 p-0 overflow-hidden rounded-2xl shadow-sm border border-slate-200">
                <ErrorBoundary><PosModule 
                  products={products} customers={customers} sales={sales} warehouses={warehouses} currentTenant={currentTenant} tenantConfig={tenantConfig} currentUser={currentUser} refreshData={() => fetchData(currentTenant)} viewingSaleId={viewingSaleId} setViewingSaleId={setViewingSaleId}
                /></ErrorBoundary>
              </div>
            )}
            ...
            {isPremium && activeTab === 'discuss' && (
              <div className="flex flex-col flex-1 w-full bg-slate-50 p-0 overflow-hidden rounded-2xl shadow-sm border border-slate-200">
                <ErrorBoundary><DiscussModule currentUser={currentUser} currentTenant={currentTenant} employees={employees} /></ErrorBoundary>
              </div>
            )}

            {isPremium && activeTab === 'documents' && (
              <div className="flex flex-col flex-1 w-full bg-slate-50 p-0 overflow-hidden rounded-2xl shadow-sm border border-slate-200">
                <ErrorBoundary><DocumentsModule currentUser={currentUser} currentTenant={currentTenant} /></ErrorBoundary>
              </div>
            )}

            {isPremium && activeTab === 'projects' && (
              <div className="flex flex-col flex-1 w-full bg-slate-50 p-0 overflow-hidden rounded-2xl shadow-sm border border-slate-200">
                <ErrorBoundary>
            ...
            {activeTab === 'history' && currentUser?.role === 'admin' && (
              <React.Suspense fallback={<div className="flex items-center justify-center h-full">Loading...</div>}>
                <ErrorBoundary><HistoryModule currentTenant={currentTenant} currentUser={currentUser} /></ErrorBoundary>
              </React.Suspense>
            )}
            ...
            {activeTab === 'superadmin' && currentUser?.email === 'ronimationstudios@gmail.com' && (
              <React.Suspense fallback={<div className="flex items-center justify-center h-full">Loading...</div>}>
                <ErrorBoundary><SuperAdminModule currentUser={currentUser} setCurrentUser={setCurrentUser} /></ErrorBoundary>
              </React.Suspense>
            )}
...
{activeTab === 'subscription' && (
          ...
          {activeTab !== 'hr' && activeTab !== 'ess' && activeTab !== 'crm' && activeTab !== 'inventory' && activeTab !== 'pos' && activeTab !== 'b2b' && activeTab !== 'analytics' && activeTab !== 'discuss' && activeTab !== 'documents' && activeTab !== 'projects' && activeTab !== 'ai' && activeTab !== 'launcher' && activeTab !== 'accounting' && activeTab !== 'history' && activeTab !== 'backend' && activeTab !== 'api' && activeTab !== 'webhooks' && activeTab !== 'subscription' && activeTab !== 'superadmin' && (
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-3 text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    ...
                    <div className="w-full overflow-hidden">
                      ...
                      <p className="text-sm font-bold text-slate-800">{selectedProfile.id.substring(0, 6).toUpperCase()}</p>
                    </div>
                  </div>
