...
export default function CrmModule({ 
  ...
  const [activeTab, setActiveTab] = useState('dashboard'); // dashboard, customers, deals, invoices, quotes, portal
  ...
  const [lastCustomerDoc, setLastCustomerDoc] = useState(null);
  ...
  const loadPaginatedCustomers = async (loadMore = false) => {
    if (!currentTenant) return;
    if (loadMore && !hasMoreCustomers) return;
    setIsLoadingCustomers(true);
    try {
      const { collection, query, orderBy, limit, startAfter, getDocs } = await import('firebase/firestore');
      const { db } = await import('./firebase');
      let q = query(collection(db, `organizations/${currentTenant}/customers`), orderBy('createdAt', 'desc'), limit(20));
      if (loadMore && lastCustomerDoc) {
        q = query(collection(db, `organizations/${currentTenant}/customers`), orderBy('createdAt', 'desc'), startAfter(lastCustomerDoc), limit(20));
      }
      const snapshot = await getDocs(q);
      const docs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setHasMoreCustomers(docs.length >= 20);
      if (snapshot.docs.length > 0) setLastCustomerDoc(snapshot.docs[snapshot.docs.length - 1]);
      setPaginatedCustomers(prev => loadMore ? [...prev, ...docs] : docs);
    } catch (err) { console.error("Error loading paginated customers:", err); }
    setIsLoadingCustomers(false);
  };

  const loadPaginatedInvoices = async (loadMore = false) => {
    if (!currentTenant) return;
    if (loadMore && !hasMoreInvoices) return;
    setIsLoadingInvoices(true);
    try {
      const { collection, query, orderBy, limit, startAfter, getDocs } = await import('firebase/firestore');
      const { db } = await import('./firebase');
      let q = query(collection(db, `organizations/${currentTenant}/invoices`), orderBy('createdAt', 'desc'), limit(20));
      if (loadMore && lastInvoiceDoc) {
        q = query(collection(db, `organizations/${currentTenant}/invoices`), orderBy('createdAt', 'desc'), startAfter(lastInvoiceDoc), limit(20));
      }
      const snapshot = await getDocs(q);
      const docs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setHasMoreInvoices(docs.length >= 20);
      if (snapshot.docs.length > 0) setLastInvoiceDoc(snapshot.docs[snapshot.docs.length - 1]);
      setPaginatedInvoices(prev => loadMore ? [...prev, ...docs] : docs);
    } catch (err) { console.error("Error loading paginated invoices:", err); }
    setIsLoadingInvoices(false);
  };

  React.useEffect(() => {
    if (activeTab === 'customers' && paginatedCustomers.length === 0) loadPaginatedCustomers();
    if (activeTab === 'invoices' && paginatedInvoices.length === 0) loadPaginatedInvoices();
  }, [activeTab]);
  ...
  // If there's a search query, fall back to the globally loaded customers (if any) or filter the paginated list locally
  const displayCustomers = searchQuery ? (customers || []).filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase())) : paginatedCustomers;
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 md:mb-6 gap-3">
        ...
        {activeTab === 'customers' && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-x-auto animate-in fade-in mb-6">
            ...
            {!searchQuery && hasMoreCustomers && displayCustomers.length > 0 && (
              <div className="p-4 text-center border-t border-slate-100">
                <button 
                  onClick={() => loadPaginatedCustomers(true)}
                ...
                >
                  {isLoadingCustomers ? 'Loading...' : 'Load More Customers'}
                </button>
              </div>
            )}
          </div>
        ...
        {activeTab === 'invoices' && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-x-auto animate-in fade-in">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {paginatedInvoices.filter(i => i.type !== 'quote').length === 0 ? (
                  <tr><td colSpan="6" className="p-8 text-center text-slate-400">No invoices found.</td></tr>
                ) : (
                  [...paginatedInvoices].filter(i => i.type !== 'quote').map(inv => {
                    const cust = (customers.length > 0 ? customers : paginatedCustomers).find(c => c.id === inv.customerId);
                    return (
            ...
            {hasMoreInvoices && paginatedInvoices.filter(i => i.type !== 'quote').length > 0 && (
              <div className="p-4 text-center border-t border-slate-100">
                <button 
                  onClick={() => loadPaginatedInvoices(true)}
                  disabled={isLoadingInvoices}
                  className="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-colors disabled:opacity-50"
                >
                  {isLoadingInvoices ? 'Loading...' : 'Load More Invoices'}
                </button>
              </div>
            )}
          </div>
                  ...
                  [...paginatedInvoices].filter(i => i.type === 'quote').map(inv => {
                    ...
                    return (
                      <tr key={inv.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="p-4 font-bold text-slate-800">{inv.currency || '$'}{Number(inv.amount).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                        <td className="p-4">
                          <span className={`px-2 py-1 rounded-lg text-xs font-bold ${inv.status === 'Accepted' ? 'bg-emerald-100 text-emerald-700' : inv.status === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
                            {inv.status || 'Draft'}
                          </span>
                        </td>
                        <td className="p-4 text-sm text-slate-500">{new Date(inv.date || Date.now()).toLocaleDateString()}</td>
            ...
            {hasMoreInvoices && paginatedInvoices.filter(i => i.type === 'quote').length > 0 && (
              <div className="p-4 text-center border-t border-slate-100">
                <button 
                  ...
                  className="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-colors disabled:opacity-50"
                >
                  {isLoadingInvoices ? 'Loading...' : 'Load More Quotes'}
                </button>
              </div>
            )}
          </div>
