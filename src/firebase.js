...
import { initializeFirestore, collection, getDocs, addDoc, doc, updateDoc, deleteDoc, serverTimestamp, setDoc, getDoc, getDocFromServer, writeBatch, runTransaction, onSnapshot, enableMultiTabIndexedDbPersistence } from 'firebase/firestore';
...
enableMultiTabIndexedDbPersistence(db).catch((err) => {
  if (err.code === 'failed-precondition') {
    console.warn('Multiple tabs open, persistence can only be enabled in one tab at a time. This is expected in development mode.');
...
export const addUserWorkspace = async (uid, tenantId, companyName, role) => {
  ...
  try {
    ...
    userSnap = await getDoc(userRef); // fallback
  }
  let workspaces = [];
  if (userSnap.exists()) {
...
export const registerTenant = async (companyName, workspaceSlug, adminEmail, adminPassword, address = '', phone = '', logoUrl = '', industry = 'retail') => {
  ...
  try {
    ...
    console.error("Error checking workspace slug:", err);
    throw new Error(`Failed to check if workspace exists: ${err.message}`);
  }
  ...
  // Create the parent organization document so it appears in the Super Admin dashboard
  const trialEnd = new Date();
  trialEnd.setDate(trialEnd.getDate() + 7);
  
  const orgRef = doc(db, 'organizations', workspaceSlug);
  await setDoc(orgRef, {
    name: companyName,
    industry: industry,
  ...
  try {
    ...
    throw new Error("Workspace created, but failed to link to your user account. Please refresh and try logging in.");
  }
...
export const getTenantConfig = async (tenantId) => {
  ...
  if (configSnap.exists()) {
    return { 
      id: tenantId, 
      ...configSnap.data(),
      // Merge parent org data for subscription info
...
export const subscribeToCollection = (tenantId, collectionName, callback, qConstraints = []) => {
  if (!tenantId) return () => {};
  // Avoid using inline import, query is already imported at the top of firebase.js
  let colRef = collection(db, `organizations/${tenantId}/${collectionName}`);
  
  // We'll let App.jsx pass in a fully constructed query object if needed, 
  // but for simplicity, we'll keep subscribeToCollection simple and handle heavy pagination 
  // via explicit getDocs calls in the modules (e.g. PosModule.jsx).
...
export const deleteChatChannel = async (channelId, tenantId = "tenant_1") => {
  await moveToHistory('chatChannels', channelId, 'Chat Channel', 'Deleted Chat Channel', tenantId);
};
